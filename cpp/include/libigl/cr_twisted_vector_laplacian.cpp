#include "cr_twisted_vector_laplacian.h"
...
IGL_INLINE void
igl::cr_twisted_vector_laplacian(
                       ...
                       Eigen::SparseMatrix<ScalarL>& L)
{
    ...
    cr_twisted_vector_laplacian_intrinsic(F, l_sq, E, oE, L);
}
                       ...
                       Eigen::SparseMatrix<ScalarL>& L)
{
    ...
    cr_twisted_vector_laplacian(V, F,
...
typename DerivedOE, typename ScalarL>
IGL_INLINE void
igl::cr_twisted_vector_laplacian_intrinsic(
                                 const Eigen::MatrixBase<DerivedF>& F,
                                 ...
                                 Eigen::SparseMatrix<ScalarL>& L)
{
    ...
    cr_twisted_vector_laplacian_intrinsic(F, l_sq, dA, E, oE, L);
}
                                 ...
                                 Eigen::SparseMatrix<ScalarL>& L)
{
    ...
    for(int f=0; f<m; ++f) {
        for(int e=0; e<3; ++e) {
            ...
            const ScalarL Dijparkipar = - 2*o*dA(f) / lens;
            tripletList.emplace_back(E(f,e), E(f,(e+2)%3), Dijparkipar);
            tripletList.emplace_back(E(f,(e+2)%3), E(f,e), Dijparkipar);
            
            const ScalarL Dijparikperp = - o * (eij - ejk + eki) / lens;
            tripletList.emplace_back(E(f,e), E(f,(e+2)%3)+nE, Dijparikperp);
            tripletList.emplace_back(E(f,(e+2)%3)+nE, E(f,e), Dijparikperp);
            tripletList.emplace_back(E(f,e)+nE, E(f,(e+2)%3), -Dijparikperp);
            tripletList.emplace_back(E(f,(e+2)%3), E(f,e)+nE, -Dijparikperp);
            
            const ScalarL Dijperpikperp = o * pow(eij - ejk + eki, 2) /
                                          (lens * 2. * dA(f));
            tripletList.emplace_back(E(f,e)+nE, E(f,(e+2)%3)+nE, Dijperpikperp);
            tripletList.emplace_back(E(f,(e+2)%3)+nE, E(f,e)+nE, Dijperpikperp);
        }
    }
    L.resize(2*nE, 2*nE);
...
template void igl::cr_twisted_vector_laplacian<Eigen::Matrix<double, -1, -1, 0, -1, -1>, Eigen::Matrix<int, -1, -1, 0, -1, -1>, Eigen::Matrix<int, -1, -1, 0, -1, -1>, Eigen::Matrix<int, -1, -1, 0, -1, -1>, double>(Eigen::MatrixBase<Eigen::Matrix<double, -1, -1, 0, -1, -1> > const&, Eigen::MatrixBase<Eigen::Matrix<int, -1, -1, 0, -1, -1> > const&, Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1> >&, Eigen::PlainObjectBase<Eigen::Matrix<int, -1, -1, 0, -1, -1> >&, Eigen::SparseMatrix<double, 0, int>&);
#endif
